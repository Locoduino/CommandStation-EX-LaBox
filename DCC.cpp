...
bool DCC::setThrottle( uint16_t cab, uint8_t tSpeed, bool tDirection)  { // tDirection true for forward, false for reverse
  ...
  if (LaboxModes::mainMode == MainMode::DCC) {
	...
	if (LaboxModes::mainMode == MainMode::DC && cab == LABOX_DC_CAB) {
	  ...
	  LaboxDC::trainSlot->setTargetSpeed(speedCode);
	
	  byte momentum=getMomentum(LaboxDC::trainSlot);
	  if (momentum && tSpeed!=1) { // not ESTOP
	    // we dont throttle speed, we just let the reminders take it to target
	    LaboxDC::trainSlot->setMomentumBase(millis());
	  } 
	  else {  // Momentum not involved, throttle now.
			LaboxDC::trainSlot->setSpeedCode(speedCode);
			LaboxDC::SetDirection(tDirection);
			LaboxDC::SetSpeed(tSpeed);	// Always change speed AFTER direction !
		}
...
void DCC::startProgTrackProcessing(int16_t cv, byte byteValue, char opType) {
	...
	#endif
	#ifdef USE_HMI
	Serial.print("DCC::startProgTrackProcessing : ");
	if (opType == 'R') {
	...
	Serial.println(menuProgMode::externalMessageBuffer);
	#endif
}
...
void DCC::endProgTrackProcessing(int16_t result) {
	...
	Serial.print("DCC::endProgTrackProcessing : ");
  ...
  switch (loopStatus) {
        ...
        case 8: {
         ...
         if (slot->getTargetSpeed() != sc) {
            ...
            //DIAG(F("Momentum factor=%d mom=%d"),MOMENTUM_FACTOR,getMomentum(slot));
